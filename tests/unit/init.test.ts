...
describe("initRegistry()", () => {
  ...
  test("writes ~/.asmrc with registry absolute path", async () => {
    const registryDir = join(tempDir, "my-registry");
    await initRegistry(registryDir);
  ...
  test("creates asm.toml with default [config] and [targets] sections", async () => {
    const registryDir = join(tempDir, "my-registry");
    await initRegistry(registryDir);
  ...
  test("creates .gitignore with .cache/ and .sync-hash entries", async () => {
    ...
    const gitignoreContent = await readFile(join(registryDir, ".gitignore"), "utf-8");
    expect(gitignoreContent).toContain(".cache/");
    expect(gitignoreContent).toContain(".sync-hash");
  });
